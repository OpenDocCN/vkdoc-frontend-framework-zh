...
动态网页和 web 应用的一个常见要求是能够向列表中添加新项。这最好通过使用`.append()`功能来实现：
...
交互式动态 web 应用和网站不仅要求 web 开发人员能够创建 DOM 元素，还要求开发人员能够添加动态内容。这可以通过另一组 jQuery 函数轻松实现。
...
这个配方通过一个非常简单的示例说明了 jQuery 开发人员可以轻松更改图像源的方式。将使用该实现的更现实的情况是在 web 应用中，例如，当用户选择他们的化身时，可以上传图像。
