...
[清单 10-1](#list1) 展示了这些效果中最简单的一个，那就是不带任何参数地使用`show`和`hide`方法。
...
当使用`finish`方法时，必须小心效果循环，比如[清单 10-18](#list18) 中的那个。为了确定所有动画属性的最终 CSS 值，`finish`方法需要执行所有的效果——尽管没有任何时间延迟——这意味着任何回调函数也要执行。
